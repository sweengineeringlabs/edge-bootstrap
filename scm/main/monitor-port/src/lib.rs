//! `swe-edge-bootstrap-monitor-port` — load-monitoring ports (traits + DTOs
//! + value objects), zero implementation.
//!
//! Consumed by `swe-edge-bootstrap-runtime-port` (`AutoscalePolicy`,
//! `MetricsConfig` embedded in `RuntimeConfig`) and
//! `swe-edge-bootstrap-metrics-port` (`SharedCounters`). The concrete
