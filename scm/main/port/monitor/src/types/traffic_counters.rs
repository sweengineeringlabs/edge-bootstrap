...
pub struct TrafficCounters {
    /// Underlying metrics provider — durable storage for exported snapshots.
    ...
    pub requests_since_tick: AtomicU64,
    /// Reset to 0 each tick by the background sampler.
    pub errors_since_tick: AtomicU64,
    /// Ring buffer of request latencies in microseconds.
    pub latency_ring: Mutex<RingBuffer>,
}
