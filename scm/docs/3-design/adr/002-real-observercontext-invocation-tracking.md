...
- **Reuse a real backend over a bespoke one.** `swe-observability-tracing`'s `TracerProvider` is already tested (811 tests across the workspace) and already partially adopted by this dependency chain (`swe_justobserv_context`'s trace-ID propagation is already live in `registry_dispatcher.rs`) — building a from-scratch span backend would duplicate infrastructure this org already operates.
